...
export interface ClientTypeFormValues {
  name: string;
}

interface ClientTypeFormModalProps {
  open: boolean;
  mode: 'create' | 'edit';
  initialData: PriceType | null;
  submitting: boolean;
  onClose: () => void;
  onSubmit: (values: ClientTypeFormValues) => void;
}

const ClientTypeFormModal: React.FC<ClientTypeFormModalProps> = ({
  open,
  mode,
  initialData,
  submitting,
  ...
  const title = mode === 'create' ? 'Nuevo Tipo de Precio' : 'Editar Tipo de Precio';
